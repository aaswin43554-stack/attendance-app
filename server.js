...
/**
 * Proxy endpoints for Supabase Updates (Bypasses Adblockers/Firewalls blocking PATCH)
 */
app.post("/api/update-user-role", async (req, res) => {
  try {
    const { email, role } = req.body;
    const supabaseUrl = process.env.VITE_SUPABASE_URL || "https://gkzsmqvzvpzjcotgqxpp.supabase.co";
    const supabaseKey = process.env.VITE_SUPABASE_ANON_KEY || "sb_publishable_MPTmQhxtVX_4DFBP6H-mwg_sjxXWSMA";
    
    const response = await fetch(`${supabaseUrl}/rest/v1/users?email=eq.${encodeURIComponent(email.toLowerCase())}`, {
      method: "PATCH",
      headers: {
        "apikey": supabaseKey,
    ...
    res.status(500).json({ error: "Failed to update role via proxy" });
  }
});

app.post("/api/assign-manager", async (req, res) => {
  try {
    ...
    const response = await fetch(`${supabaseUrl}/rest/v1/users?email=eq.${encodeURIComponent(email.toLowerCase())}`, {
      ...
      body: JSON.stringify({ managed_by: managerEmail })
    });
    ...
    res.status(500).json({ error: "Failed to assign manager via proxy" });
  }
});
